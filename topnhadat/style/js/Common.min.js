...
function getInterest(n, t, i) {
    $.ajax({
        type: "POST",
        cache: !1,
        url: "/interest/getterm?v=" + Math.random(),
        data: {
            act: "InterestBank",
            bank: n,
            term: t,
    ...
    listProductId != null ? (likeReturn = "", $("#boxProductSaved ul").html(html), $("body").data("GetProductlist_" + listProductId) != null ? (likeReturn = $("body").data("GetProductlist_" + listProductId), $.each(likeReturn, function(n, t) {
        ...
        success: function(data) {
            data != null && (likeReturn = eval("(" + data + ")"), $("body").data("GetProductlist_" + listProductId, likeReturn), $.each(likeReturn, function(n, t) {
                html += '<li pid="' + t.ProductId + '"><a href="' + t.SourceNews + '">' + t.Title + '<\/a><span title="Xóa" onclick="deleteProduct(this,' + t.ProductId + ');"><\/span><\/li>';
                $('#TinDaLuu').append('<tr class="saveIMG' + t.ProductId + '"><td>' + parseInt(n+1) + '</td><td>' + t.ProductId + '</td><td><div class="mng-prdsave">' + t.Title + '</div><div class="mng-action"><a href="' + t.SourceNews + '" class="mng-action-view" target="_blank">Xem</a></div></td><td style=""><div><div class="product-action"><a onclick="deleteProduct(this,' + t.ProductId + ');" title="Xóa" class="action-del" href="javascript://">Xóa</a></div></td></tr>');
                $('#TinDaLuu2').append('<div class="data-item saveIMG' + t.ProductId + '"><div class="data-row" style="width:23%">' + t.ProductId + ' <br></div><div class="data-row" style="width:48%"><div class="product-info"><p><a href="' + t.SourceNews + '" style="font-weight: bold; color: green">' + t.Title + '</a></p> </div><div class="clearboth"></div></div><div class="data-row" style="width:23%"><div class="product-action product-info"><span class="wr-action"></span><div class="pro-action"><div class="product-action"><a class="action-view" href="' + t.SourceNews + '" target="_blank">Xem</a> <a onclick="deleteProduct(this,' + t.ProductId + ');" class="action-del" href="javascript://">Xoá</a></div></div></div></div></div>');
            }), html != "" ? ($("#boxProductSaved").show(), $("#boxProductSaved ul").html(html)) : ($.removeCookie("savedProductIds", {
                domain: domainCookie,
                path: "/"
...
$(document).ready(function() {
    ...
    $("body").on("click", "#Email_Register_Right", function() {
        ...
        if (n.indexOf(" ") < 0 && /.+@.+\.[a-zA-Z]{2,4}$/.test(n)) {
            var t = $("#hddcboType").val(),
                ...
                u = $("#hddcboDist").val(),
                f = $("#hddcboProject").val(),
                e = "/Services/SearchService?pType=" + t + "&cateId=" + i + "&cityCode=" + r + "&distId=" + u + "&projId=" + f + "&EmailRegister=" + n;
    ...
    $("body").on("click", "#UtilityRegisterEmail", function() {
        $("#UtilityRegisterEmail").attr("href", "/Services/SearchService");
    ...
    $("body").on("click", "#UtilityFengShui", function() {
        $("#UtilityFengShui").attr("href", "/chi-phi-lan-banh");
    ...
    $("body").on("click", "#UtilityInterest", function() {
        $("#UtilityInterest").attr("href", "/uoc-tinh-vay-ngan-hang");
    ...
    $("body").on("click", "#InterestRateForm", function() {
        ...
        n && $.ajax({
            url: "/interest/getinterest?v=" + Math.random(),
            data: {
                act: "InterestRate",
                total: t,
                borrowingTime: i,
    ...
    if (window.location.pathname == "/" ? ($("#ban_l1").show(), $("#ban_l2").show(), $("#ban_r1").show(), $("#ban_r2").show()) : $("#ban_l1").show(), (window.location.pathname == "/dang-nhap.htm" || window.location.pathname == "/bao-gia.htm") && ($("#ban_l1").show(), $("#ban_l2").hide(), $("#ban_r1").show(), $("#ban_r2").hide()), $("#SiteLeft").show(), $("#SiteRight").show(), e = ".ban_scroll .item", e.length > 0) {
        var n = 1e3,
            r = $("#SiteLeft .ban_scroll").width(),
            o = $("#SiteRight .ban_scroll").width(),
        ...
        function f() {
            ...
            var f = 0,
                t = $(window).scrollTop(),
                e = window.location.pathname != "/" ? 192 : 440,
                i = 0,
                h = $(".footer").position().top - $(".ban_scroll").height();
            i = t <= e ? e : t >= h ? h : t - f + 0;
            $("#SiteLeft .ban_scroll .item").length != 0 && $("#SiteLeft .ban_scroll").stop().animate({
                top: i + 20,
                left: u
            }, 400);
            $("#SiteRight .ban_scroll .item").length != 0 && $("#SiteRight .ban_scroll").stop().animate({
                top: i + 20,
                right: u
            }, 400);
            f = t
    ...
    t = $(".inews .inews-l .inews-l-content").height();
    i = $(".inews .inews-r .inews-l-content").height();
    t != i && (t > i ? i = t : t < i && (t = i), $(".inews .inews-l .inews-l-content").height(t), $(".inews .inews-r .inews-l-content").height(i))
});
ForgotPassword1 = function() {
    validateEmailForgot() && ($("#rcloadding").show(), $.ajax({
        ...
        type: "POST",
        success: function(n) {
            if ($("#rcloadding").hide(), n.success == 0) return $("#spanMessage").html(n.mess), !1;
            $("#spanMessage").html(n.mess)
        }
    }))
};
validateEmailForgot = function() {
    if ($("#txtEmailForgot").val() == null || $("#txtEmailForgot").val() == "") return $("#spanMessage").html("Email không được để trống!"), $("#txtEmailForgot").focus(), !1;
    if ($("#spanMessage").html(""), $("#txtEmailForgot").val() == "" || /.+@.+\.[a-zA-Z]{2,4}$/.test($("#txtEmailForgot").val())) $("#spanMessage").html("");
    else return $("#spanMessage").html("Không đúng định dạng email.(joe@example.com)"), $("#txtEmailForgot").focus(), !1;
    return $("#txtEmailForgot").focus(), !0
};
MemberLogOut = function() {
    if (confirm("Bạn có chắc chắn muốn thoát ?")) {
        var n = $('<iframe id="ifLogout" src="/dang-xuat.htm"/>');
        $("body").append(n);
        n.load(function() {
...
String.prototype.insert = function(n, t) {
    return n > 0 ? this.substring(0, n) + t + this.substring(n, this.length) : t + this
};
String.prototype.replaceAll = function(n, t) {
    for (var i = this, r = i.indexOf(n); r != -1;) i = i.replace(n, t), r = i.indexOf(n);
    return i
};
