...
class Dangtin extends MX_Controller {
    ...
    foreach ($image_sizes as $resize) {
        ...
        $this->image_lib->resize();
            ...
            //
            if($Loai!='logo'){  
            echo '<div class="sortableitem"><a href="'.show_img($img['file_name']).'" title="" rel="prettyPhoto[gallery1]"><img src="'.show_img($img['file_name']).'"></a> <a class="del" href="javascript:void(0)" title="Xóa ảnh" id="'.$img['file_name'].'" onclick="return del_img(this)"><i class="mpx-close"></i></a></div><input type="hidden" id="imgID" value="'.$img['file_name'].'"/>';
            }else{
        ...
        if($vip->{'solandangtin'}-$sotinban<=0)
        {
            echo '<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
            echo '<script>alert("Bạn đã sử dụng hết số lần đăng tin hôm nay !\n Hãy đăng ký thành viên VIP hoặc SIÊU VIP để đăng đc nhiều tin mỗi ngày hơn.");window.location="'.base_url().'dang-ky-thanh-vien-vip.html";</script>';
    ...
    function xemdanhmuc($Loai='',$ID='',$page=1,$tinhthanh='') {
        ...
        $dieukien=array('tinban.TrangThai'=>1);
        ...
        include(APPPATH . 'includes/phantrang.php');//print_r($dieukien);
        $data['dulieu']=$this->default_model->getTableRows($this->_table,$config['per_page'],$start,'level = \'xsieuvip_user\' desc, level = \'vip_user\' desc, uptin desc, IDMaTin desc','',$this->_id,$noibang,$dieukien);
    ...
    function quanlytinban($page=1,$active=1){
        if($this->session->userdata('userid')==FALSE){
            redirect(base_url().'thanh-vien/dang-nhap.html');
            exit();
        }
        $dieukien=array("TrangThai" => $active, "NguoiDang" => $this->session->userdata('userid'));
    ...
    function uptin($ID) {
        ...
        {
            echo '<script>alert("Bạn đã hết số lượt up tin hôm nay !");window.location="/thanh-vien/quan-ly-tin-rao";</script>';
        ...
        $xx=$this->default_model->updateDuLieu($this->_table,array('uptin'=>date('Y-m-d H:i:s',time())),$truyvan);
       	if($xx){echo '<script>alert("Bạn đã up tin thành công !");window.location="/thanh-vien/quan-ly-tin-rao";</script>'; }
    }
    ...
    function update()
    {
        ...
        foreach($arr_fills as $parent=>$link){
         ...
         $base='https://batdongsantop10.net';$stt=0;
         $get=$base.'/'.$link; 
         ...
         if(isset($tit11[7])){
          ...
          if(!$check){ 
               ...
               $getcity='https://batdongsantop10.net/Handler/SearchHandler.ashx?module=';
               ...
               $mmLink='https://img.batdongsantop10.net/crop/624x476/';
